...
# 1.리스트(List) ex)기차! [ 0 1 2 3 ] [ -4 -3 -2 -1 ]
# - 시퀀스 자료형 (연속 된 값 저장)
# - 대가로 사용[]
# - 정렬 가능
...
#패킹과 언패킹
list_d = ["A", "B", "C"]  #패킹    **패킹과 언패킹 수가 동일해야 함!!
a, b, c = list_d          #언패킹
#a = list_d[0]
#b = list_d[1]
#c = list_d[2]
...
a.insert(1, "A") #(인덱스, 값)  *값이 지워지지 않고 값을 밀고 insert!!
print(a)
...
a = [1, 2, 3]
print(a.index(3)) #인덱스 반환  **괄호 안에 값이 들어가는지 인덱스가 들어가는지!!**
...
b = {"b" :3,
     "c": 5}
a.update(b) #병합시 중복 key가 있는 경우 입력값(b)이 우선!
print(a)
...
# get() : 값 접근
# list, tuple, dict 접근 -> 컬렉션[index of key] ex: a[2]
